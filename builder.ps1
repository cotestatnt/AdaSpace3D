...
# ============================================================
Write-Host ""
Write-Host "Would you like to open the Serial Monitor for debugging?" -ForegroundColor Yellow
Write-Host "  Press 'Y' to open Serial Monitor, any other key to exit." -ForegroundColor Yellow
Write-Host ""

$key = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
if ($key.Character -eq 'y' -or $key.Character -eq 'Y') {
    Write-Host ""
    Write-Step "6" "Serial Monitor"
    ...
    if ($ports.Count -eq 0) {
        Write-Warning-Custom "No COM ports found. Device may still be restarting."
        Write-Info "You can open a serial monitor manually at 115200 baud."
    }
    elseif ($ports.Count -eq 1) {
        $port = $ports[0]
        Write-Info "Found port: $port"
        Write-Info "Opening serial monitor at 115200 baud..."
        Write-Host ""
        Write-Host "  Press Ctrl+C to exit the serial monitor." -ForegroundColor DarkGray
        Write-Host ""
        ...
        Write-Info "Multiple ports found:"
        for ($i = 0; $i -lt $ports.Count; $i++) {
            Write-Host "  [$i] $($ports[$i])" -ForegroundColor Cyan
        }
        Write-Host ""
        $selection = Read-Host "Enter port number (0-$($ports.Count - 1))"
        
        try {
            $portIdx = [int]$selection
            if ($portIdx -ge 0 -and $portIdx -lt $ports.Count) {
                $port = $ports[$portIdx]
                Write-Info "Opening serial monitor on $port at 115200 baud..."
                Write-Host ""
                Write-Host "  Press Ctrl+C to exit the serial monitor." -ForegroundColor DarkGray
                Write-Host ""
                Write-Host "============================================================" -ForegroundColor Cyan
                ...
                Write-Warning-Custom "Invalid selection."
            }
        }
        catch {
            Write-Warning-Custom "Invalid input."
        }
    }
}
